...
$( function () {
    ...
    $("#national_id_value").val(localStorage.getItem("uin"));
...
async function searchFunction(){
    ...
    $("#registerDiv" ).on("click",function(){
        location.href = "BankDetails.html";
    });

    var count = 0;
    localStorage.setItem("demodata", null);
    ...
    var MainURL = "http://13.59.128.159:3005/getdatamediator"                                  
    options.url = MainURL;
...
function forDetailData(ajaxResponse,tableBody,dataForsearch,trcount){
    ...
    &&  dataForsearch.identifier_id !=null && dataForsearch.identifier_id!=""){
        $.each(ajaxResponse.data.data,   function(key,value) {
            ...
            if(value.detailData !=null && value.detailData!=undefined && 
                ...
                value.detailData.father.name[0].firstNames !=null && value.detailData.father.name[0].firstNames !=undefined ) {
                    fatherName =  value.detailData.father.name[0].firstNames + " " +value.detailData.father.name[0].familyName;
            }
            if(value.detailData !=null && value.detailData!=undefined && 
                ...
                value.detailData.mother.name[0].firstNames !=null && value.detailData.mother.name[0].firstNames !=undefined ) {
                    motherName =  value.detailData.mother.name[0].firstNames + " " + value.detailData.mother.name[0].familyName;
            }
            ...
            if(  responseValue.identifier !=null && responseValue.identifier !=undefined  ) {
                ...
                if(typeOfRecord== "Death"){
                    ...
                    jsonObject["registrationNumber"] = registrationNumber;  
                    jsonObject["identiferId"] = indetifeirType ; 
                    jsonObject["identiferType"] = identiferId ;                     
                    jsonObject["typeOfRecord"] = typeOfRecord;
                    jsonObject["given"] = responseValue.name[0].given[0];
                    jsonObject["family"] = responseValue.name[0].family[0] ; 
                    jsonObject["fatherName"] = fatherName;
                    jsonObject["motherName"] = motherName;
                    jsonObject["deceasedDate"] = responseValue.deceasedDateTime;
                    jsonObject["contactNumber"] ="";
                    jsonObject["is_alive"] = "No" ;

                    DeathArrayObject.push(jsonObject);

                    var  markupTr = "<tr>" 
                    +"<td><input type='radio' name='radiobutton' name='radiobutton"+totalcountsTR+"' "
                    +" data='"+JSON.stringify(jsonObject)+"' /></td>"                     
                    +"<td>"  + (indetifeirType!=null && indetifeirType!=undefined && indetifeirType!="" ?identiferId:"")   + "</td>"
                    +"<td>"  +  indetifeirType + "</td>"
                    +"<td class='scenario1' ></td>"
                    +"<td class='scenario1' >"  + registrationNumber + "</td>"         
                    +"<td class='scenario1' >"  + typeOfRecord   + "</td>"
                    +"<td>"  + responseValue.name[0].given[0]   + " "+ responseValue.name[0].family[0]   +"</td>"
                    +"<td>"  + fatherName  + "</td>"
                    +"<td>"  +  motherName   + "</td>"
                    +"<td></td>"
                    +"<td></td>"
                    ...
                    jsonObject["fatherName"] = fatherName;
                    jsonObject["motherName"] = motherName;
                    jsonObject["dateOfBirth"] = responseValue.birthDate ;
                    jsonObject["contactNumber"] ="";
                    jsonObject["deceasedDate"] = null;
                    jsonObject["is_alive"] = "Yes" ;
                    ...
                    +"<td>"  +  motherName  + "</td>"
                    +"<td></td>"                       
                    +"<td>"  + ageCalculator(responseValue.birthDate , null)    + "</td>"
                    +"<td>True</td>" 
                    +"</tr>";
        ...
        if(value.type == "Death"){
                ...
                var jsonObject = {};
                jsonObject["registrationNumber"] = value.registration.registrationNumber ;                   
                jsonObject["typeOfRecord"] = value.type ;
                jsonObject["identiferId"] = indetifeirType ;  
                jsonObject["identiferType"] = identiferId ;       
                jsonObject["given"] = value.deceasedName[0].firstNames ;
                jsonObject["family"] = value.deceasedName[0].familyName;
                jsonObject["fatherName"] = fatherName;
                jsonObject["motherName"] = motherName;
                jsonObject["deceasedDate"] = value.dateOfDeath  ;
                jsonObject["contactNumber"] = value.registration.contactNumber ;
                jsonObject["is_alive"] = "No";
                ...
                +"<td>"  +  indetifeirType + "</td>"
                +"<td class='scenario1' ></td>"
                +"<td class='scenario1' >"  + value.registration.registrationNumber   + "</td>" 
                +"<td class='scenario1' >"  + value.type   + "</td>"
                +"<td>"  + value.deceasedName[0].firstNames  + " " + value.deceasedName[0].familyName   +  "</td>" 
                +"<td>"  + fatherName  + "</td>"
                +"<td>"  +  motherName   + "</td>"
            ...
            if(value.birthIdentifier !=null && value.birthIdentifier!=undefined && 
                ...
                jsonObject["identiferType"] = identiferId ;               
                jsonObject["typeOfRecord"] = value.type ;
                jsonObject["given"] = value.childName[0].firstNames  ;
                jsonObject["family"] = value.childName[0].familyName  ;
                jsonObject["fatherName"] = fatherName;
                jsonObject["motherName"] = motherName;
                jsonObject["dateOfBirth"] = value.dateOfBirth  ;
                jsonObject["contactNumber"] = value.registration.contactNumber ;
                jsonObject["deceasedDate"] = null;
                jsonObject["is_alive"] = "Yes" ;
                ArrayObject.push(jsonObject);
                ...
                +"<td class='scenario1' ></td>"
                +"<td class='scenario1' >"  + value.type   + "</td>"
                +"<td>"  + value.childName[0].firstNames   + " " + value.childName[0].familyName + "</td>"
                
                +"<td>"  + fatherName + "</td>"
                +"<td>"  +  motherName  + "</td>"
                +"<td>"  + value.registration.contactNumber   + "</td>"                       
                +"<td>"  + ageCalculator(value.dateOfBirth, null)   + "</td>"
                +"<td>True</td>" 
                +"</tr>";
     ...
     $.each(DeathArrayObject, function(key,value) {
        $.each(ArrayObject, function(key1,value1) {
            
            if( value.identiferId == value1.identiferId){
                debugger
                value1.is_alive = "No";
                value1["deceasedDate"] = value.deceasedDate;
                debugger
             }
          }); 
      }); 
      $.each(ArrayObject, function(key1,value1) {
        ...
        var  markupTr = "<tr>"
      + (value1.is_alive =="Yes" ? "<td><input type='radio' name='radiobutton' name='radiobutton"+totalcountsTR+"' "
        +" data='"+JSON.stringify(value1)+"' /></td>" : "<td></td>") 
        +"<td>"  + (value1.identiferId!=null && value1.identiferId!=undefined && value1.identiferId!="" ?value1.identiferType:"")   + "</td>"
        +"<td>"  +  (value1.identiferId!=null && value1.identiferId!=undefined ?value1.identiferId:"")+ "</td>"                  
        +"<td class='scenario1' >"  + value1.identiferId  + "</td>" 
        +"<td class='scenario1' ></td>"
        +"<td class='scenario1' ></td>"
        +"<td>"  + value1.given   + " " + value1.family+ "</td>"
        
        +"<td>"  + value1.fatherName + "</td>"
        +"<td>"  +  value1.motherName  + "</td>"
        +"<td>"  + value1.contactNumber  + "</td>"                       
        +"<td>"  + ageCalculator(value1.dateOfBirth, value1.deceasedDate)   + "</td>"
        +"<td>" + value1.is_alive+"</td>" 
        +"</tr>";
        totalcountsTR++;
        trcount++;
        tableBody.append(markupTr);
        $("#totalrecordsinput").val(  totalcountsTR );
        $("#totalrecords").text("Total Records found : "+$("#totalrecordsinput").val());
      }); 
...
function addNoRecordsfound(){
    $("#searchedRecords").append('<tr><td colspan="11" style="text-align: center;">No Records Found</td></tr>');
}
...
function onChangeRadioButton(){
    ...
    $('input[type=radio][name=radiobutton]').off("change").on("change",function() {
       var data= $('input[name="radiobutton"]:checked').attr("data");
...
let myPromise =    new Promise( function(resolve, reject) {
 ...
 function ageCalculator(userinput, userdod) {    
    var dob = new Date(userinput);  
    var dod = new Date();
    if(userdod!=null && userdod!=undefined) {
        dod = new Date(userdod);
    }
    //calculate month difference from current date in time  
    var month_diff = dod.getTime() - dob.getTime();  
