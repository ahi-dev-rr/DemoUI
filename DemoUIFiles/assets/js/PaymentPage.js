...
$( function () {
    ...
    if(demodata!=null && demodata!="" && demodata!=undefined){
        ...
        var BankDetailsString = demodataJson.backDetail.toString();
        var bankvalue = "";
        for(var i=0;i<BankDetailsString.length-2;i++){
         ...
         bankvalue= bankvalue + BankDetailsString.slice(BankDetailsString.length - 2, BankDetailsString.length)
            ...
            var    markupTr = "<tr id='tr_41'>"      
                    +"<td>"+dataCheck(demodataJson.identiferId)+"</td>"
                    +"<td>"+dataCheck(demodataJson.given) + " " +dataCheck(demodataJson.family)+"</td>" 
                    +"<td>"+dataCheck(demodataJson.contactNumber)+"</td>"
                    +"<td>"+dataCheck(bankvalue)+"</td>"
                    +"<td>5500</td>"     
                    +"</tr>";
        ...
        if(markupTr!=null && markupTr!="" && markupTr!=undefined){
                tableBody = $("#paymentRecords");
                tableBody.append(markupTr); 

                $("#tr_41").css("background-color", "#e5e5e5");
                setTimeout(function() {
                          $("#tr_41").css("background-color", "white");
                  }, 2000);
                $("#tr_41").toggleClass("hidebg").fadeIn(3000, function() {
                     $(this).toggleClass("hidebg").fadeOut(10)
                })
                $("#tr_41").toggleClass("hidebg").fadeIn();
...
$("#paymentRecords").find('tr'). each (function (index, td) {
    console.log($(this).children('td:first'). text());
    var nationalid = $(this).children('td:first'). text();
    var object =   {
              idtype: "NATIONAL_ID",
             idvalue: nationalid,
             amount: "5500",
              currency: "INR" }
              payeearray.push(object);
    });
    ...
    $.ajax( options )
     .done( function( ajaxResponse ) {
        ...
        localStorage.setItem("paymentrespose", JSON.stringify(ajaxResponse));
        ...
        $('#gAlertModal').on('hidden.bs.modal', function () {
            location.href = "PaymentHistory.html";
        }); 
        gOnPageLoad();
     });
